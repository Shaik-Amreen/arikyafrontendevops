    ...
    if (this.formgroupdata.status == 'VALID') {
      ...
      this.commonservice.postrequest('http://localhost:4000/findoneusers', this.formgroupdata.value).subscribe(
        (res1: any) => {
          ...
          (res1.role == 'admin') ? this.router.navigate(['/admin']) : (res1.role == 'student') ? ((res1.login == '') ? this.router.navigate(['/studentform']) : this.router.navigate(['/student/studenthome'])) : (res1.role == 'faculty') ? this.router.navigate(['/faculty/home']) : null
        },
